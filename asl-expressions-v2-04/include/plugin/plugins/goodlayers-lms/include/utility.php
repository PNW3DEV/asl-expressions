<?php
	...
	*/	
	...
	function gdlr_lms_excerpt_more( $more ) {
		return '... <div class="clear"></div><a href="' . get_permalink() . '" class="excerpt-read-more">' . __( 'Learn More', 'gdlr-lms' ) . '</a>';
	}	
	...
	function gdlr_lms_get_pagination($max_num_page, $current_page, $format = 'paged'){
		...
		return 	'<div class="gdlr-pagination">' . paginate_links(array(
			...
			'total' => $max_num_page,
			'prev_text'=> __('&lsaquo; Previous', 'gdlr-lms'),
			'next_text'=> __('Next &rsaquo;', 'gdlr-lms')
		)) . '</div>';
	}		
	...
	function gdlr_lms_get_thumbnail_list(){
		...
		$sizes['full'] = __('full size (Original Images)', 'gdlr-lms');
