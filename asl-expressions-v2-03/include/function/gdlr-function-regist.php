...
<?php			
		...
		function gdlr_get_default_admin_option() {
			...
			if(empty($sidebar_option)){
				update_option('gdlr_sidebar_name', 
					array( 'blog', 'blog-left', 'portfolio', 'portfolio-left', 'shortcodes',  'woocommerce',  'Features', 'Archives', 'contact', 'Course', 'LMS')
				);
			}
			
			$google_font_list = get_option(THEME_SHORT_NAME . '_google_font_list', array());
			if( empty($google_font_list) ){
				$font_text  = 'a:2:{s:7:"Raleway";a:2:{s:7:"subsets";a:1:{i:0;s:5:"latin";}s:8:"variants";a:9:{i:0;s:3:"100";i:1;s:3:"200";i:2;s:3:"300";i:3;s:7:"regular";i:4;s:3:"500";i:5;s:3:"600";i:6;s:3:"700";i:7;s:3:"800";i:8;s:3:"900";}}s:10:"Montserrat";a:2:{s:7:"subsets";a:1:{i:0;s:5:"latin";}s:8:"variants";a:2:{i:0;s:7:"regular";i:1;s:3:"700";}}}';
	...
	if( !function_exists('gdlr_excerpt_more') ){
		function gdlr_excerpt_more( $more ) {
			...
			return '... <div class="clear"></div><a href="' . get_permalink() . '" class="gdlr-button with-border excerpt-read-more">' . __( 'Read More', 'gdlr_translate' ) . '</a>';
		}
	}
	...
	<article id="comment-<?php comment_ID(); ?>" class="comment-article">
		...
		<div class="comment-body">
			...
			<div class="comment-reply">
				<?php comment_reply_link( array_merge($args, array('before' => ' <i class="fa fa-mail-reply icon-mail-reply"></i>', 'reply_text' => __('Reply', 'gdlr_translate'), 'depth' => $depth, 'max_depth' => $args['max_depth'])) ); ?>
			</div><!-- reply -->					
			</header>
...
<li class="gdlr-mega-menu">
	<?php 
		if(is_user_logged_in()){
			echo '<a href="' . wp_logout_url(get_permalink()) . '"><i class="fa fa-lock icon-lock"></i>' . __('Logout', 'gdlr_translate') . '</a>';
			...
			echo '<a href="#"><i class="fa fa-lock icon-lock"></i>' . __('Login', 'gdlr_translate') . '</a>';
	?>
	<div class="sf-mega">
...
?>
<li class="gdlr-mega-menu">
	<a href="#"><i class="fa fa-envelope icon-envelope"></i><?php echo __('Subscribe', 'gdlr_translate'); ?></a>
	<div class="sf-mega">
